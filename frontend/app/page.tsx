...
import { MessageCircle, Bot, Zap, Clock, Users, TrendingUp } from 'lucide-react';

// Use your test influencer ID
...
export default function Dashboard() {
    ...
    if (loading) return (
        <div className="min-h-screen bg-gray-950 flex items-center justify-center">
            <div className="text-white text-xl animate-pulse">Loading ReplyPilot AI...</div>
        </div>
    );
        ...
        <div className="min-h-screen bg-gray-950 text-white">
            ...
            <div className="border-b border-gray-800 px-8 py-4 flex items-center justify-between">
                <div className="flex items-center gap-3">
                    ...
                    <h1 className="text-xl font-bold">ReplyPilot AI</h1>
                </div>
            ...
            <div className="px-8 py-6">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                    <StatCard
                        ...
                        value={overview?.overview?.total_comments || 0}
                        color="blue"
                    />
                    <StatCard
                        icon={<Bot size={20} />}
                        label="AI Handled"
                        value={overview?.breakdown?.ai_handled || 0}
                        sub={`${overview?.breakdown?.ai_percentage || 0}%`}
                        color="purple"
                    />
                    <StatCard
                        icon={<Zap size={20} />}
                        label="Rule Handled"
                        value={overview?.breakdown?.rule_handled || 0}
                        sub={`${overview?.breakdown?.rule_percentage || 0}%`}
                        color="yellow"
                    />
                    <StatCard
                        icon={<Clock size={20} />}
                        label="Pending Review"
                        value={overview?.overview?.pending_review || 0}
                        color="orange"
                    />
                </div>
                                        ...
                                        <div className="flex items-center gap-2">
                                            <div className="w-20 h-2 bg-gray-800 rounded-full overflow-hidden">
                                                <div
                                                    ...
                                                    style={{
                                                        width: `${Math.min((value / (overview?.overview?.total_comments || 1)) * 100, 100)}%`
                                                    }}
                                                />
                                            </div>
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="flex justify-between text-sm">
                                    ...
                                    <span className="text-yellow-400 font-medium">
                                        {overview?.overview?.collab_requests || 0}
                                    </span>
                                </div>
                            </div>
                        </div>
